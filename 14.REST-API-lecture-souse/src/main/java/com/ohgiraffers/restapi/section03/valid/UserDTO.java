...
public class UserDTO {
    ...
    @NotNull(message = "id 는 반드시 입력되어야한다.")
    @NotBlank(message = "아이디는 공백일 수 없다.")
    private String id;
    private String pwd;
    @NotNull(message = "이름은 반드시 적어야한다.")
    @Size(min = 2, message = "이름은 두글자 이상 입력해야한다.")
    private String name;
    @Past
    ...
    public @NotNull(message = "id 는 반드시 입력되어야한다.") String getId() {
        return id;
    }

    public void setId(@NotNull(message = "id 는 반드시 입력되어야한다.") String id) {
        this.id = id;
    }
